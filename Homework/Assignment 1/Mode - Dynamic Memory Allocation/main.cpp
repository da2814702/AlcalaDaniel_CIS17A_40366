 ...
 * Edited by: Daniel Alcala on 3/22/2020
 */
...
int *mode(const int *array,int arySize){
    ...
    int currentElement = -1;//the current element
    int currentFreq = 0;//current frequency
    int nmodes;//number of modes
    
    for(int i = 0; i < arySize; ++i)
    {
        if(currentElement==ary[i])
        {
            currentFreq++;
    ...
    modeAry[0]=nmodes;//Stub returns no modes - now sets first element
                      //to the number of modes
    modeAry[1]=maxFreq;//Stub returns Frequency 1 - now sets the second
                       //element equal the max frequency
   
    //Fill the mode array
    currentElement = -1;
    currentFreq = 0;
    int idx = 2;//index set at 2 to skip modes and frequency
    for (int i = 0; i < arySize; ++i)
    {
        if(currentElement == ary[i])
        {
